...
class VisionTransformerEF(VisionTransformer):
    ...
    def forward_features(self, z, x,z_event,x_event, mask_z=None, mask_x=None,
        ...
        x = self.cross_block(x, x_event)
        z = self.cross_block(z, z_event)
    ...
    def forward(self, z, x, z_event, x_event, ce_template_mask=None, ce_keep_rate=None,
        ...
        x, aux_dict = self.forward_features(z, x, z_event=z_event, x_event=x_event,
                                             ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate,)

        return x, aux_dict

class VisionTransformerMF(VisionTransformer):
    """ Vision Transformer with candidate elimination (CE) module
    ...
    def forward(self, z, x, z_event, x_event, ce_template_mask=None, ce_keep_rate=None,
        ...
        x_event, aux_dict_event = self.forward_features(z_event, x_event, ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate,)
        x = self.cross_block(x, x_event)
        
        return x, aux_dict

class VisionTransformerLF(VisionTransformer):
    """ Vision Transformer with candidate elimination (CE) module
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.event=VisionTransformerCE(img_size=img_size, patch_size=patch_size, in_chans=in_chans, num_classes=num_classes, embed_dim=embed_dim, depth=depth,
                 num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, representation_size=representation_size, distilled=distilled,
                 drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, drop_path_rate=drop_path_rate, embed_layer=embed_layer, norm_layer=norm_layer,
                 act_layer=act_layer, weight_init=weight_init, ce_loc=ce_loc, ce_keep_ratio=ce_keep_ratio)
    ...
    def forward(self, z, x, z_event, x_event, ce_template_mask=None, ce_keep_rate=None,
        ...
        x, aux_dict = self.vis(z, x, ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate,)
        x_event, aux_dict_event = self.event(z_event, x_event, ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate,)
...
class VisionTransformerHWMF(VisionTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.ce_loc = ce_loc
        for i in range(depth):
            if cross_index==10 or cross_index==11:
                blocks.append(
            ...
            # if cross_index==10 or cross_index==11:
            #     blocks.append(
            #         CoCrossBlock(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            #         attn_drop=attn_drop_rate, norm_layer=norm_layer, act_layer=act_layer)
            #     )
            else:
                blocks.append(
...
def vit_hybrid_patch16_224_ce_ef(pretrained=False, **kwargs):
