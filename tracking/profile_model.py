...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='ostrack', choices=['ostrack','test'],
...
def evaluate_vit_hybrid(model, template, search):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, search, template, search),
                             custom_ops=None, verbose=False)
    macs, params = clever_format([macs1, params1], "%.3f")
    print('overall macs is ', macs)
    print('overall params is ', params)
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search, template, search)
        start = time.time()
        for i in range(T_t):
            _ = model(template, search, template, search)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    if args.script == "ostrack":
        ...
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_test
        model = model_constructor(cfg, training=False)
        # get the template and search
        template = torch.randn(bs, 3, z_sz, z_sz)
        search = torch.randn(bs, 3, x_sz, x_sz)
        ...
        evaluate_vit_hybrid(model, template, search)
